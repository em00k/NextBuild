...
dim C,XO,YO as uinteger
dim offset,kx as float 
dim ca as byte 
dim Color, cc,nc, W, H,rx,X, Y,value,XX,YY,u,v,o,p,a,b as ubyte 
nc = 0 : offset = 17
R = 127 : R2 = R * R : H = 192 : W = 255
ClipLayer2(0,254,0,191)
for kx = 0 to 6.28 step 0.05
...
For X = 0 To W-1
				...
				For Y = 0 To 191
						...
						'	FOR YY=0 to 191 step 64
						'		FOR XX=0 to 3
								C = C - ( cast(uinteger,( XO - YO)) ) 
								
							Color = cast(ubyte,(C  ) )  
							PlotL2(X,Y,Color)
						'		NEXT 
						'	NEXT 
				YO=YO+2 : if YO>126 : YO = 0 : ENDIF 
        Next
				XO=XO+2 : if XO>124 : XO = 0 : ENDIF 
				'XO=XO+2 : if XO>63 : XO = 0 : ENDIF 
				YO=0
Next
